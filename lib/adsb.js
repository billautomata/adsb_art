...
// @billautomata ripped it from - from https://github.com/grantmd/node-adsb
...
exports.decodePacket = function(data){
	...
	if (DEBUG) console.log('Flight Status '+msg.fs+': '+this.flightStatusToString(msg.fs));
	...
	if (msg.type === 0 || msg.type == 4 || msg.type == 16 || msg.type == 20){
		...
		if (ac13){
			...
			msg.unit = ac13.unit;
			if (DEBUG) console.log('AC13 Altitude: '+msg.altitude+', Units: '+((msg.unit == MODES_UNIT_FEET) ? 'ft' : 'm'));
		}
		else{
			if (DEBUG) console.log('Could not decode AC13');
		}
	}
	...
	if (msg.msgtype == 17){
		...
		if (msg.metype >= 1 && msg.metype <= 4){
			...
			if (ac12){
				...
				msg.unit = ac12.unit;
				if (DEBUG) console.log('AC12 Altitude: '+msg.altitude+', Units: '+((msg.unit == MODES_UNIT_FEET) ? 'ft' : 'm'));
			}
			else{
				if (DEBUG) console.log('Could not decode AC12');
			}
